...
var localeData = {}; //This one is loaded by the locale manager
...
function locale_reloadStrings()
{

}

function locale_load(locale)
{
		...
		success: 
			function(data, textStatus, jqXHR)
			{
				if(data.LOCALE_VERSION == TIGRIS_VERSION)
				{
					...
					locale_reloadStrings();
				}else
				{
					ui_showError("The definition of the locale '" + locale + "' is not valid.");
				}
			},
		error:
			function(jqXHR, textStatus, errorThrown)
			{
				ui_showError("The definition of the locale '" + locale + "' could not be found.");
			}
...
function locale_get(string)
{
	...
	var data = localeData[string] || "missing token";
...
function ui_init()
{
	...
	ui_showError(locale_get("MSG_CONNECTING")); //Connecting to the server...
}
...
{
	var error = msg || "An unknown error occurred"; //ERR_UNKNOWN
...
function ui_i_login()
{
	...
	packet.onResponse = 
		function(pk)
		{
			...
			{
				ui_login_showError("Your login data is incorrect", true); //MSG_LOGIN_BADLOGINDATA
			}
		};
...
function netio_handshake()
{
	...
	function(pk)
	{
		...
		}else if(pk.typeID == PTYPE.ERROR)
		{
			...
			ui_showError("Connection refused by server"); //ERR_COMM_CONNECTIONREFUSED
		}
	};
